 ...
 */
public class KMedoids extends ClusteringAlg {
     ...
     */
    public KMedoids(DataSet dset, CombinedMetric cmet, int numClusters) {
        setDataSet(dset);
     ...
     * @param numClusters A pre-defined number of clusters.
     */
    public KMedoids(DataSet dset, int numClusters) {
        setDataSet(dset);
    ...
    public void cluster() throws Exception {
        ...
        for (int cIndex = 0; cIndex < medoids.length; cIndex++) {
            medoidsIndex = randa.nextInt(dset.size());
            while (clusterAssociations[medoidsIndex] != -1) {
                medoidsIndex = randa.nextInt(dset.size());
            }
            clusterAssociations[medoidsIndex] = cIndex;
            medoids[cIndex] = dset.getInstance(medoidsIndex).copyContent();
        }
